...
YoloInferencer::YoloInferencer(const std::string& modelPath, const char* logid, const char* provider)
    ...
    //Ort::Model model_(env_, modelPath.c_str(), Ort::ModelLoggingLevel::ORT_WARN);
    //session_ = Ort::Session(env_, model_, sessionOptions);
    ...
    if (names_item != metadata.end()) {
        ...
        // todo: print class id and class name for debugging
...
std::vector<Detection> YoloInferencer::postprocess(std::vector<Ort::Value>& outputTensors, float conf_threshold, float iou_threshold) {
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        if (max_conf > conf_threshold) {
            ...
            if (names_.count(class_id.x)){
                class_names.push_back(names_[class_id.x]);
            }else{
                class_names.push_back("unknown");
            }

            // get the box dimensions of the detected object
            float out_w = pdata[2];
            float out_h = pdata[3];
            float out_left = std::max((pdata[0] - 0.5f * out_w), 0.0f);
            float out_top = std::max((pdata[1] - 0.5f * out_h), 0.0f);

            cv::Rect_<float> bbox = cv::Rect_<float>(out_left, out_top, (out_w + 0.5), (out_h + 0.5)); // x, y, width, hight of the box
